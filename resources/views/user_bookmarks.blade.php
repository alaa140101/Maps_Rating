<x-app-layout>
  ...
  <div class="container mx-auto md:px-12 p-5">
    <h3 class="mb-4 mt-4 text-2xl"> الإشارات المرجعية</h3>
    <hr>
    @if (!count($bookmarks))
        <x-alert color="blue" message="لاتوجد أي إشارات مرجعية بعد" />
    @endif

    @foreach ($bookmarks as $place)
        <div class="flex mt-5 bg-white border">
          <div class="flex-none w-48 relative">
            <img src="{{ $place->image }}" alt="Place" class="absolute inset-0 w-full h-full object-cover">
          </div>
          <div class="flex-auto p-6">
            <div class="flex flex-wrap">
              <h1 class="flex-auto text-xl font-semibold">{{ $place->name }}</h1>
              <div class="w-full flex-none text-sm font-medium text-gray-500 mt-2">{{ $place->address }}</div>
            </div>
